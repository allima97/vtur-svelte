      ...
      loader: async () => {
        ...
        const items = Array.from(byDestino.values())
          .map((item) => ({
            ...
            destino_slug: String(item.destino ?? "")
              .toLowerCase()
              .replace(/\s+/g, "-")
              .replace(/[^a-z0-9\\-]/g, ""),
            destino_codigo: String(item.destino ?? "")
              .replace(/[^A-Za-z0-9]/g, "")
              .toUpperCase(),
            ticket_medio:
              item.quantidade > 0 ? item.receita / item.quantidade : 0,
            percentual:
              totalReceita > 0 ? (item.receita / totalReceita) * 100 : 0,
          }))
