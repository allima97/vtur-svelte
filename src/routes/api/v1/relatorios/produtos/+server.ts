...
export async function GET(event) {
  try {
    ...
    rows.forEach((row) => {
      ...
      .map((item) => {
        const produtoId = typeof (item as any).produto_id !== 'undefined' ? (item as any).produto_id : null;
        const produtoNameSlug = String(item.produto ?? '').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9\-]/g, '');
