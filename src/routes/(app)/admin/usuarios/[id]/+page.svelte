<script lang="ts">
  ...
  async function saveUser() {
    ...
    try {
      ...
      if (!isCreateMode && !userForm.uso_individual && !userForm.company_id) {
      ...
      if (isCreateMode && payload.id) {
        await goto(`/admin/usuarios/${payload.id}`);
      } else {
        await loadDetail();
      }
    } catch (err) {
...
<svelte:head>
  <title>{isCreateMode ? 'Novo usuario' : 'Usuario'} | VTUR</title>
</svelte:head>

<PageHeader
  title={isCreateMode ? 'Novo usuario' : userMeta?.nome || 'Usuario'}
  subtitle={isCreateMode
    ? 'Cadastro administrativo com papel, empresa, escopo e senha inicial.'
    : 'Edicao administrativa do usuario, com acoes de acesso e seguranca.'}
  breadcrumbs={[
  ...
  actions={
    isCreateMode
      ...
      : [
          ...
          { label: 'Permissoes', href: `/admin/permissoes/${currentId}`, variant: 'outline', icon: ShieldCheck }
        ]
  }
/>
  ...
  <Card color="financeiro" title="Cadastro administrativo">
    <div class="grid gap-4 lg:grid-cols-2">
      ...
      <div>
        ...
        <select id="usuario-empresa" bind:value={userForm.company_id} class="vtur-input w-full" disabled={userForm.uso_individual}>
          <option value="">Selecione</option>
    ...
    <div class="grid gap-6 xl:grid-cols-2">
      <Card color="financeiro" title="Permissoes aplicadas">
        <div class="space-y-3">
          ...
          <Button variant="outline" href={`/admin/permissoes/${currentId}`}>Abrir editor completo</Button>
        </div>
      </Card>
