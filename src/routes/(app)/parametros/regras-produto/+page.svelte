<script lang="ts">
  ...
  const columns = [
    {
      ...
      label: 'Produto',
      sortable: true,
      formatter: (_v: unknown, row: RegraProduto) => row.tipo_produtos?.nome || '—'
    },
    {
      key: 'commission_rule',
      ...
      sortable: true,
      width: '160px',
      formatter: (_v: unknown, row: RegraProduto) =>
  ...
  async function load() {
    ...
    loading = true;
    try {
      const [itemsPayload, produtosPayload, regrasPayload] = await Promise.all([
        apiGet<{ items?: RegraProduto[] }>('/api/v1/parametros/regras-produto', undefined, request.signal),
      ...
      toast.error(toUserMessage(err, 'Erro ao carregar regras por produto.'));
    } finally {
  ...
  function openEdit(row: RegraProduto) {
    editingId = row.id;
    form = {
  ...
  async function save() {
    ...
    saving = true;
    try {
      await apiPost('/api/v1/parametros/regras-produto', {
      ...
      toast.error(toUserMessage(err, 'Erro ao salvar regra por produto.'));
    } finally {
      saving = false;
    }
  }
  ...
  async function deleteItem(id: string) {
    ...
    deletingId = id;
    try {
      await apiDelete('/api/v1/parametros/regras-produto', { id });
...
<svelte:head>
  <title>Regras por Produto | VTUR</title>
</svelte:head>

<PageHeader
  title="Regras por Produto"
  subtitle="Vincule regras de comissão ou percentuais fixos a produtos específicos."
  color="financeiro"
  breadcrumbs={[
...
<DataTable
  ...
  searchable={true}
  emptyMessage="Nenhuma regra por produto cadastrada"
...
<Dialog
  bind:open={modalOpen}
  title={editingId ? 'Editar Regra por Produto' : 'Nova Regra por Produto'}
  color="financeiro"
  size="lg"
  showCancel={true}
  cancelText="Cancelar"
  ...
  <div class="space-y-4">
    ...
    <div class="rounded-xl border border-financeiro-200 bg-financeiro-50/40 p-4 space-y-4">
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        <FieldInput id="rp-fix-nao" label="% meta não atingida" type="number" bind:value={form.fix_meta_nao_atingida} placeholder="0" class_name="w-full" suffix="%" />
        <FieldInput id="rp-fix-atingida" label="% meta atingida" type="number" bind:value={form.fix_meta_atingida} placeholder="0" class_name="w-full" suffix="%" />
        <FieldInput id="rp-fix-super" label="% super meta" type="number" bind:value={form.fix_super_meta} placeholder="0" class_name="w-full" suffix="%" />
      </div>
    </div>
  </div>
</Dialog>
