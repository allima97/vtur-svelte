    ...
    {#if currentStep === 0}
      <FormPanel title="Dados da venda" description="Atualize os dados principais da venda" class_name="border-green-200">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Lançada em</label>
            <input type="date" bind:value={venda.data_lancamento} class="vtur-input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Data da venda *</label>
            <input type="date" bind:value={venda.data_venda} class="vtur-input w-full" class:border-red-500={errors.data_venda} />
          ...
          <div>
            ...
            <input type="date" bind:value={venda.data_embarque} class="vtur-input w-full" class:border-red-500={errors.data_embarque} />
            {#if errors.data_embarque}<p class="mt-1 text-xs text-red-600">{errors.data_embarque}</p>{/if}
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Data final *</label>
            <input type="date" bind:value={venda.data_final} class="vtur-input w-full" class:border-red-500={errors.data_final} />
            {#if errors.data_final}<p class="mt-1 text-xs text-red-600">{errors.data_final}</p>{/if}
          </div>

          <div class="xl:col-span-2">
            <label class="mb-1 block text-sm font-medium text-slate-700">Destino principal *</label>
            <select bind:value={venda.destino_id} class="vtur-input w-full" class:border-red-500={errors.destino_id}>
              <option value="">Selecione...</option>
    ...
    {#if currentStep === 2}
      ...
      <FormPanel title="Resumo e observações" description="Confira totais, status e notas internas" class_name="border-green-200">
        ...
        <div class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Valor total da venda</label>
            <input bind:value={venda.valor_total} class="vtur-input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Valor total bruto</label>
            <input bind:value={venda.valor_total_bruto} class="vtur-input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Valor total pago</label>
            <input bind:value={venda.valor_total_pago} class="vtur-input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-700">Valor não comissionado</label>
            <input bind:value={venda.valor_nao_comissionado} class="vtur-input w-full" />
          </div>
        </div>
