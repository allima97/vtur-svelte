<script lang="ts">
  ...
  async function handleAprovar() {
    ...
    const redirectToVenda = await confirmAction(
      "Deseja seguir agora para criar a venda a partir deste orçamento?",
    );
...
<svelte:head>
  <title
    >{orcamento ? `Orçamento ${orcamento.codigo}` : "Orçamento"} | VTUR</title
  >
</svelte:head>
...
{:else if orcamento}
  ...
  <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
    <div>
      ...
      <p class="text-sm text-slate-500">
        Resumo do orçamento com foco em status comercial, follow-up, validade e
        conversão em venda.
      </p>
    </div>
  </div>
  ...
  <div class="vtur-kpi-grid mb-6">
    ...
    >
      <div
        class={`flex h-10 w-10 items-center justify-center rounded-xl ${statusAtual === "aprovado" ? "bg-green-50 text-green-500" : statusAtual === "rejeitado" ? "bg-red-50 text-red-500" : statusAtual === "enviado" ? "bg-blue-50 text-blue-500" : "bg-amber-50 text-amber-500"}`}
      >
        <FileText size={20} />
      </div>
      <div>
    ...
    >
      ...
      <div>
        ...
        <p class="text-2xl font-bold text-slate-900">
          {!ultimaInteracao ? "Sem registro" : `${diasSemInteracao || 0}d`}
        </p>
      </div>
    </Button>
    ...
    >
      <div
        class={`flex h-10 w-10 items-center justify-center rounded-xl ${isExpirado ? "bg-red-50 text-red-500" : "bg-amber-50 text-amber-500"}`}
      >
        <AlertCircle size={20} />
      </div>
      <div>
    ...
    >
      <div
        class={`flex h-10 w-10 items-center justify-center rounded-xl ${orcamentoConvertido ? "bg-green-50 text-green-500" : podeCriarVenda ? "bg-green-50 text-green-500" : "bg-slate-100 text-slate-500"}`}
      >
    ...
    <div class="flex items-center justify-between flex-wrap gap-4">
      <div class="flex items-center gap-3 flex-wrap">
        ...
        {#if orcamentoConvertido}
          <span
            class="px-2 py-1 bg-emerald-200 text-emerald-800 text-xs font-medium rounded-full"
          >
            CONVERTIDO
          </span>
        {/if}
  ...
  <div class="mb-6 rounded-lg border px-4 py-3 {cardProximoPassoClasse}">
    {#if orcamentoConvertido}
      ...
      <p class="mt-1 text-sm">
        Este orçamento já foi convertido. Use esta tela como histórico
        comercial.
      </p>
    {:else if podeCriarVenda}
      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p class="mt-1 text-sm">
            O orçamento já está aprovado e pronto para conversão operacional.
          </p>
        </div>
        <Button
          variant="secondary"
      ...
      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <p class="text-sm font-semibold">
            Próximo passo: registrar o primeiro contato
          </p>
          <p class="mt-1 text-sm">
            Este orçamento ainda não tem interação registrada e entrou na fila
            crítica de follow-up.
          </p>
        </div>
        <Button
          variant="secondary"
    ...
    {:else if (diasSemInteracao || 0) >= 7}
      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p class="mt-1 text-sm">
            Faz {diasSemInteracao} dias desde a última interação. Vale reabrir contato
            agora.
          </p>
        </div>
        <Button
    ...
    {:else}
      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p class="mt-1 text-sm">
            O orçamento está com interação recente. Mantenha o acompanhamento
            até a definição comercial.
          </p>
        </div>
        <Button
          variant="secondary"
  ...
  <div class="vtur-kpi-grid mb-6">
    ...
    >
      <div
        class="flex h-10 w-10 items-center justify-center rounded-xl {isExpirado
    ...
    <div class="vtur-kpi-card">
      ...
      <div>
        ...
        <p class="text-2xl font-bold text-slate-900">
          {orcamento.currency === "USD"
            ? "US$"
            : orcamento.currency === "EUR"
              ? "€"
              : "R$"}
        </p>
      </div>
            ...
            <table class="w-full table-mobile-cards">
              <thead>
                <tr class="border-b border-slate-200">
                  <th
                    class="text-left py-3 px-3 text-sm font-semibold text-slate-600"
                    >Descrição</th
                  >
                  <th
                    class="text-center py-3 px-3 text-sm font-semibold text-slate-600"
                    >Tipo</th
                  >
                  <th
                    class="text-center py-3 px-3 text-sm font-semibold text-slate-600"
                  ...
                  >
                  <th
                    class="text-right py-3 px-3 text-sm font-semibold text-slate-600"
                    >Valor Unit.</th
                  >
                  <th
                    class="text-right py-3 px-3 text-sm font-semibold text-slate-600"
                ...
                {#each orcamento.itens as item}
                  <tr class="hover:bg-slate-50">
                    <td class="py-3 px-3 text-slate-900">
                      <p class="font-medium">
                        {item.title || item.descricao || "Item"}
                      </p>
                    ...
                    <td class="py-3 px-3 text-center">
                      <span
                        class="px-2 py-1 text-xs bg-slate-100 rounded-full capitalize"
                      >
                        {item.item_type || "servico"}
                      </span>
                    </td>
                    <td class="py-3 px-3 text-center text-slate-700"
                      >{item.quantity || item.quantidade || 1}</td
                    >
                    <td class="py-3 px-3 text-right text-slate-700">
                      {formatCurrency(
                        item.unit_price || item.valor_unitario || 0,
                      )}
                    </td>
                    <td class="py-3 px-3 text-right font-medium text-slate-900">
                      {formatCurrency(
                        item.total_amount || item.valor_total || 0,
                      )}
                    </td>
                  </tr>
                {/each}
              ...
              <tfoot>
                <tr class="border-t-2 border-slate-200">
                  <td
                    colspan="4"
                    class="py-4 px-3 text-right font-semibold text-slate-900"
                    >Total do Orçamento:</td
                  >
                  <td
                    class="py-4 px-3 text-right text-xl font-bold text-clientes-600"
                  >
                    {formatCurrency(orcamento.total || valorTotal)}
                  </td>
                </tr>
              ...
              <div class="p-3 bg-slate-50 rounded-lg">
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    {#if interacao.status}
                      <span
                        class="inline-block mt-2 px-2 py-0.5 text-xs rounded-full {getStatusColor(
                          interacao.status,
                        )}"
                      >
