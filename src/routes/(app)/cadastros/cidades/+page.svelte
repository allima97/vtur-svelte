<script lang="ts">
  ...
  async function load() {
    ...
    // Não carrega tudo por padrão; só busca quando houver critério.
  ...
  onMount(() => {
    // Modo sob demanda: não pré-carrega cidade/subdivisão sem busca/filtro.
...
<Dialog bind:open={modalOpen} title={editingId ? 'Editar Cidade' : 'Nova Cidade'} size="sm" showCancel={true} cancelText="Cancelar" showConfirm={true} confirmText={editingId ? 'Salvar' : 'Criar'} loading={saving} onConfirm={save} onCancel={() => (modalOpen = false)}>
  <div class="space-y-4">
    ...
    <FieldSelect
      ...
      placeholder={null}
      class_name="w-full"
      disabled={loadingSubdivisoes || subdivisoes.length === 0}
    />
    <FieldInput
      id="cid-desc"
      label="Descrição"
