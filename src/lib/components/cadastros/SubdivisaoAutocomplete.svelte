<script lang="ts">
  ...
  async function ensureSelectedLoaded(subdivisaoId: string) {
    ...
    try {
      ...
      // Mantemos o campo utilizável mesmo sem conseguir carregar o valor atual.
    } finally {
  ...
  async function searchOptions(term: string) {
    ...
    loading = true;
    try {
      const payload = await apiGet<any>('/api/v1/subdivisoes', { q: query });
      if (currentQuery !== query) return;
      const items = Array.isArray(payload?.items) ? payload.items : [];
      results = uniqueOptions([
        ...(value ? results.filter((item) => String(item.id) === String(value)) : []),
        ...sortOptions(items, query).slice(0, maxResults)
      ]);
