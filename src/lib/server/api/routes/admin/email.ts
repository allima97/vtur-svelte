// Migrado para Hono de src/routes/api/v1/admin/email/+server.ts — corpo IDÊNTICO ao original
...
export async function handleAdminEmailGet(event: RequestEvent) {
  try {
...
export async function handleAdminEmailPost(event: RequestEvent) {
  try {
    const originError = rejectCrossOriginRequest(event.request);
    if (originError) return originError;
    const bodyResult = await readJsonBodyLimited(event.request, MAX_EMAIL_SETTINGS_BODY_BYTES);
