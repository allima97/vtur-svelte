// Migrado para Hono de src/routes/api/v1/convites/accept/+server.ts — corpo IDÊNTICO ao original
...
export const handleConvitesAcceptPost = async (event: RequestEvent) => {
  try {
    const { request } = event;
