...
`);
  ...
  it('não classifica como Seguro Viagem um pacote que apenas inclui seguro junto com hotel', async () => {
    const result = await extractContratosFromText(`
Contrato de Intermediação de Serviços de Turismo
...
Nº Contrato: 5630-0000084441 Reserva: 304779615
Excursão: 9.06087465.26062102
Destino: - ROMA  -  3 dia(s) /  2 noite(s)
Data Saída: 21/06/2026
Data Retorno: 23/06/2026
...
2 DIÁRIAS NO LEONARDO BOUTIQUE HOTEL ROME MONTI - DIARIAS EM APARTAMENTO (Sem Café Da Manhã) (Sem Café da Manhã)
TIPO ACOMODAÇÃO: 1 APARTAMENTO COZY ROOM - HOTELBEDS - NR: 207-16121213 - [207-16121213]
SEGURO VIAGEM COMPRE JA NACIONAL PLUS
...
`);
  ...
  it('classifica como Seguro Viagem quando o seguro é o único serviço do contrato', async () => {
    const result = await extractContratosFromText(`
Contrato de Intermediação de Serviços de Turismo
...
Nº Contrato: 5630-0000084442 Reserva: 304779616
Excursão: 9.06087465.26062103
Destino: - ROMA  -  3 dia(s) /  2 noite(s)
Data Saída: 21/06/2026
Data Retorno: 23/06/2026
...
4. DO PRECO
5. VALOR E FORMA DE PAGAMENTO
Os serviços contratados totalizam o valor de R$  314,03
`);

    expect(result.contratos).toHaveLength(1);
    expect(result.contratos[0].produto_tipo).toBe('Seguro viagem');
  });
});
