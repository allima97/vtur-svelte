...
function normalizePaymentText(value?: string | null) {
  return normalizeText(value, { collapseWhitespace: true, trim: true });
}
...
async function carregarTermosNaoComissionaveis(client: any): Promise<string[]> {
  try {
    ...
    const termos = (data || [])
      .map((row: any) => normalizePaymentText(row?.termo_normalizado || row?.termo))
      .filter(Boolean);

    return termos.length > 0 ? Array.from(new Set(termos)) : DEFAULT_NAO_COMISSIONAVEIS.map(normalizePaymentText);
  } catch {
    return DEFAULT_NAO_COMISSIONAVEIS.map(normalizePaymentText);
  }
}

function isFormaNaoComissionavel(nome?: string | null, termos?: string[]) {
  const normalized = normalizePaymentText(nome);
  if (!normalized) return false;
  if (normalized.includes('cartao') && normalized.includes('credito')) return false;
  const base = termos && termos.length > 0 ? termos : DEFAULT_NAO_COMISSIONAVEIS.map(normalizePaymentText);
