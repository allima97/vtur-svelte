<script lang="ts">
  ...
  import { formatCurrency, formatDate, formatMonthYearLabel } from '$lib/utils/formatters';
  ...
  function formatMonthLabel(monthKey: string) {
    ...
    return formatMonthYearLabel(year, month, 'short');
  }
  ...
  const autoReload = createDebouncedReload(() => void loadRelatorio(), 250);
  ...
  const _autoReloadKey = () => [
    filtroPeriodoModo, mesSelecionado, dataInicio, dataFim,
    empresaSelecionada, vendedorSelecionado, clienteIdFiltro,
    destinoFiltro, produtoFiltro, tipoProdutoFiltro
  ];

  async function handleFilterChange() {
    await tick();
    autoReload.enable(_autoReloadKey());
    void loadRelatorio();
  }
  ...
  $: autoReload.watch(_autoReloadKey());
