<script lang="ts">
  ...
  async function syncSessionOnServer(session: LoginSession) {
    return apiPost('/api/auth/set-session', {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      const payload = await apiPost<{ session: LoginSession; user: unknown }>('/api/auth/login', {
        email,
        password,
  ...
  async function handlePasskeyLogin() {
    ...
    try {
      const optionsPayload = await apiPost<{ options: unknown; challengeId: string }>(
        '/api/auth/passkeys/login/options',
        { email: email.trim() || null },
        undefined
      );

      const assertion = await startAuthentication({ optionsJSON: optionsPayload.options as any });
      const payload = await apiPost<{ session: LoginSession; user: unknown }>(
        '/api/auth/passkeys/login/verify',
        { challengeId: optionsPayload.challengeId, response: assertion },
        undefined
      );
