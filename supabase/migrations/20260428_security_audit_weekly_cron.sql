-- 2026-04-28: auditoria semanal de seguranca via pg_cron
-- Cria tabela de resultados, funcao de auditoria e agendamento semanal.
...
begin
  ...
  -- B) Policies amplas (qual/with_check = true), excluindo allowlist intencional
  with broad as (
  ...
  -- D) Estado RLS/force em tabelas criticas (conta somente as que nao estao corretas)
  with critical as (
    select
      n.nspname as schema_name,
      c.relname as table_name,
...
-- Agenda semanal: segunda-feira, 06:00 (UTC)
do $$
declare
  v_jobid bigint;
begin
