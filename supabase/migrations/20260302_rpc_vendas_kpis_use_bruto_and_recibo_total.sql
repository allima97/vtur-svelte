-- 2026-03-02: corrigir RPCs de KPIs/Dashboard para:
-- - Usar vendas.valor_total_bruto (quando existir) como referência de faturamento bruto
-- - Considerar bruto do recibo = valor_total (já inclui taxas + RAV)
-- - Taxas efetivas (não comissionáveis) = valor_taxas - valor_du
-- - Aplicar o mesmo fator de proporcionalidade também nas taxas e no seguro
...
with vendas_filtradas as (
  select
    v.id,
    coalesce(v.valor_total_bruto, v.valor_total, 0) as valor_total_ref,
    coalesce(v.valor_taxas, 0) as valor_taxas
...
recibos as (
  select
    ...
    sum(greatest(0, coalesce(r.valor_taxas, 0) - coalesce(r.valor_du, 0))) as total_taxas,
    sum(
      case
...
calc as (
  select
    v.id,
    case
      when r.total_bruto is null then v.valor_total_ref
      else
        (case
          when r.total_bruto > 0 and v.valor_total_ref > 0 then
            least(1, greatest(0, v.valor_total_ref / r.total_bruto))
          else 1
        end) * r.total_bruto
    end as vendas_total,
    case
      when r.total_bruto is null then v.valor_taxas
      else
        (case
          when r.total_bruto > 0 and v.valor_total_ref > 0 then
            least(1, greatest(0, v.valor_total_ref / r.total_bruto))
          else 1
        end) * coalesce(r.total_taxas, 0)
    end as taxas_total,
    case
...
recibos_base as (
  select
    ...
    greatest(0, coalesce(r.valor_taxas, 0) - coalesce(r.valor_du, 0)) as valor_taxas,
...
ratio as (
  select
    ...
    case
      ...
      when rs.total_bruto > 0 and v.valor_total_ref > 0 then
        least(1, greatest(0, v.valor_total_ref / rs.total_bruto))
      else 1::numeric
    end as fator
  from vendas_filtradas v
...
calc_venda as (
  select
    ...
    v.data_venda,
    case
      when rs.total_bruto is null then v.valor_total_ref
      else ratio.fator * rs.total_bruto
    end as vendas_total,
    case
      when rs.total_bruto is null then v.valor_taxas
      else ratio.fator * coalesce(rs.total_taxas, 0)
    end as taxas_total,
    case
...
select
  ...
  (select jsonb_agg(jsonb_build_object('day', day, 'total', total)) from timeline_rows) as timeline,
  (select jsonb_agg(jsonb_build_object('nome', nome, 'total', total)) from destinos_rows) as top_destinos,
  (select jsonb_agg(jsonb_build_object('id', id, 'nome', nome, 'total', total)) from produtos_rows) as por_produto,
  (select jsonb_agg(jsonb_build_object('vendedor_id', vendedor_id, 'total', total, 'qtd', qtd)) from vendedor_rows) as por_vendedor
from kpis k;
$$;
