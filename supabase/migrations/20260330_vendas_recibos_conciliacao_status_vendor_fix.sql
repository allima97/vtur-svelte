-- 2026-03-30: ajusta a RPC de status de conciliacao para vendedores sem uso_individual=true
-- O vendedor deve enxergar o status dos seus proprios recibos mesmo quando o perfil
-- estiver marcado apenas como VENDEDOR no tipo de usuario.
...
as $$
  ...
  visible_receipts as (
    ...
    where
      ...
      or (
        not public.is_admin(a.uid)
        and not public.is_master(a.uid)
        and not public.is_gestor(a.uid)
        and v.company_id = a.company_id
