...
async function GET(event) {
  try {
    ...
    const { data, error } = await client.from("cidades").select("*").eq("id", cidadeId).single();
    if (error) {
...
async function PATCH(event) {
  try {
    ...
    const updateData = {};
    if (body.nome !== void 0) updateData.nome = body.nome.trim();
    if (body.descricao !== void 0) updateData.descricao = body.descricao?.trim() || null;
    if (Object.keys(updateData).length === 0) {
      return json({ error: "Nenhum campo para atualizar." }, { status: 400 });
    }
    const { data, error } = await client.from("cidades").update(updateData).eq("id", cidadeId).select().single();
    if (error) {
...
async function DELETE(event) {
  try {
    ...
    const cidadeId = event.params.id;
    const { error } = await client.from("cidades").delete().eq("id", cidadeId);
    if (error) {
