...
async function GET(event) {
  try {
    ...
    const { data, error: queryError } = await client.from("system_module_settings").select("module_key, enabled, reason").limit(200);
    if (queryError) {
    ...
    const rows = data || [];
    const disabled = rows.filter((r) => r.enabled === false).map((r) => r.module_key);
    return json({
      table_missing: false,
...
async function POST(event) {
  try {
    ...
    const { data: existing } = await client.from("system_module_settings").select("id").eq("module_key", module_key).maybeSingle();
    const payload = {
      module_key,
