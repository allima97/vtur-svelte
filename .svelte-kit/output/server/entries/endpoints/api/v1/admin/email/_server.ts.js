...
const MASKED = "••••••";
function ensureAdminOnly(scope) {
  if (!scope.isAdmin) {
    throw error(403, "Somente ADMIN pode editar configuracoes globais.");
  }
}
...
async function POST(event) {
  try {
    ...
    ensureAdminOnly(scope);
    const existingCreds = await client.from("admin_email_settings").select("id, smtp_pass, resend_api_key").eq("singleton", true).maybeSingle();
    ...
    if (existingCreds.data?.id) {
      const { error: updateError } = await client.from("admin_email_settings").update(payload).eq("id", existingCreds.data.id);
