...
async function GET(event) {
  try {
    ...
    const { data: categorias } = await client.from("crm_template_categories").select("id, nome, icone, sort_order, ativo").order("sort_order").limit(100);
    const { data: temas } = await client.from("user_message_template_themes").select("id, nome, categoria_id, asset_url, scope, greeting_text, mensagem_max_linhas, mensagem_max_palavras, assinatura_max_linhas, assinatura_max_palavras, ativo").order("nome").limit(200);
    const { data: templates } = await client.from("user_message_templates").select("id, nome, categoria, titulo, corpo, scope, ativo").order("nome").limit(200);
    return json({
