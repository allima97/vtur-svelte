...
import { t as toErrorResponse, g as getAdminClient, r as requireAuthenticatedUser, a as resolveUserScope, e as ensureModuloAccess } from "../../../../../../chunks/v1.js";
...
async function requireAccess(event, minLevel) {
...
function canAccessRuleCompany(companyId, allowedCompanyIds) {
...
async function GET(event) {
  try {
    ...
    let data = null;
    let error = null;
    const primary = await client.from("commission_rule").select("*, company_id, created_by, commission_tier(*)").order("nome", { ascending: true });
    data = primary.data;
    error = primary.error;
    if (error && isMissingColumnError(error)) {
      const fallback = await client.from("commission_rule").select("*, commission_tier(*)").order("nome", { ascending: true });
      data = fallback.data;
...
async function POST(event) {
  try {
    ...
    const payload = {
      ...
      ativo: body?.ativo === void 0 ? true : Boolean(body?.ativo)
    };
    ...
    if (persistedId) {
      ...
      let insertResult = await client.from("commission_rule").insert(payloadWithScope).select("id").single();
    ...
    const { error: deleteError } = await client.from("commission_tier").delete().eq("rule_id", persistedId);
    if (deleteError) {
      throw deleteError;
    }
    if (tiers.length > 0) {
