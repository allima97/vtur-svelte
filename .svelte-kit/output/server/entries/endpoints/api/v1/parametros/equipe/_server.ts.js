...
async function GET(event) {
  try {
    ...
    let usersQuery = client.from("users").select("id, nome_completo, email, active, uso_individual, company_id, user_types(name)").eq("active", true).order("nome_completo");
    ...
    let convitesQuery = client.from("user_convites").select("id, invited_email, company_id, user_type_id, invited_by, invited_by_role, status, created_at, expires_at").eq("status", "pending");
    if (scope.companyId && !scope.isAdmin) {
    ...
    const { data: tiposData } = await client.from("user_types").select("id, name").order("name").limit(50);
    return json({
      usuarios: usersData || [],
      relacoes: relData || [],
...
async function POST(event) {
  try {
    ...
    if (action === "toggle_relacao") {
      ...
      const { data: existing } = await client.from("gestor_vendedor").select("id, ativo").eq("gestor_id", gestorId).eq("vendedor_id", vendedor_id).maybeSingle();
      if (existing) {
        const { error: updateError } = await client.from("gestor_vendedor").update({ ativo: ativo !== false }).eq("id", existing.id);
        if (updateError) throw updateError;
      } else {
        const { error: insertError } = await client.from("gestor_vendedor").insert({ gestor_id: gestorId, vendedor_id, ativo: ativo !== false });
