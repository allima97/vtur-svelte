...
async function GET(event) {
  try {
    ...
    const { data, error: queryError } = await client.from("quote_print_settings").select("id, owner_user_id, company_id, logo_url, logo_path, imagem_complementar_url, imagem_complementar_path, consultor_nome, filial_nome, endereco_linha1, endereco_linha2, endereco_linha3, telefone, whatsapp, whatsapp_codigo_pais, email, rodape_texto").eq("owner_user_id", user.id).maybeSingle();
