...
async function GET(event) {
  try {
    ...
    let query = client.from("tipo_produtos").select("id, nome, tipo, descricao, ativo, soma_na_meta, regra_comissionamento, usa_meta_produto, meta_produto_valor, comissao_produto_meta_pct, descontar_meta_geral, exibe_kpi_comissao, created_at").order("nome", { ascending: true }).limit(200);
    ...
    if (error && (String(error.code || "").includes("42703") || String(error.message || "").includes("does not exist"))) {
      const fallback = await client.from("tipo_produtos").select("id, nome, tipo, descricao, ativo, created_at").order("nome", { ascending: true }).limit(200);
      if (!fallback.error) {
        data = fallback.data;
        error = null;
      }
