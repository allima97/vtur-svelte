...
async function GET(event) {
  try {
    ...
    let query = client.from("sac_controle").select("id, company_id, recibo, tour, data_solicitacao, motivo, contratante_pax, ok_quando, status, responsavel, prazo, created_at, updated_at").order("created_at", { ascending: false }).limit(500);
    ...
    if (q) {
      ...
      items = items.filter(
        (item) => [item.recibo, item.tour, item.motivo, item.contratante_pax, item.responsavel].join(" ").toLowerCase().includes(qLower)
      );
    }
    return json({ items });
  } catch (err) {
