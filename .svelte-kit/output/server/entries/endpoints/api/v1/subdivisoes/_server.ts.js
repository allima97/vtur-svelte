...
async function GET(event) {
  try {
    ...
    if (!scope.isAdmin) {
      ensureModuloAccess(scope, ["cadastros"], 1, "Sem acesso a Estados/Subdivisões.");
    }
    const { searchParams } = event.url;
    const q = String(searchParams.get("q") || "").trim();
    const paisId = String(searchParams.get("pais_id") || "").trim();
    let query = client.from("subdivisoes").select("id, nome, pais_id, codigo_admin1, tipo, created_at, pais:paises!pais_id(id, nome)").order("nome").limit(2e3);
...
async function POST(event) {
  try {
    ...
    const scope = await resolveUserScope(client, user.id);
    if (!scope.isAdmin) {
      ensureModuloAccess(scope, ["cadastros"], 2, "Sem permissão para salvar estados.");
    }
...
async function DELETE(event) {
  try {
    ...
    const scope = await resolveUserScope(client, user.id);
    if (!scope.isAdmin) {
      ensureModuloAccess(scope, ["cadastros"], 4, "Sem permissão para excluir estados.");
    }
