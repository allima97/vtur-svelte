...
async function POST(event) {
  try {
    ...
    const { data, error } = await client.from("documentos_viagens").update({ display_name: displayName, updated_at: (/* @__PURE__ */ new Date()).toISOString(), updated_by: user.id }).eq("id", id).select("id, file_name, display_name, storage_bucket, storage_path, mime_type, size_bytes, created_at, updated_at").maybeSingle();
