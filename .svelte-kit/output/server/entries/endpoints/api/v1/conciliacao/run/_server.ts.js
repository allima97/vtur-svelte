...
import { g as getAdminClient, r as requireAuthenticatedUser, a as resolveUserScope, e as ensureModuloAccess, b as resolveScopedCompanyIds, t as toErrorResponse, i as isUuid } from "../../../../../../chunks/v1.js";
async function reconcilePendentes(client, companyId, limit, conciliacaoReciboId) {
  let query = client.from("conciliacao_recibos").select("id, documento, movimento_data, descricao, status, valor_lancamentos, valor_calculada_loja").eq("company_id", companyId).eq("conciliado", false).limit(limit);
  ...
  for (const row of rows) {
    ...
    const { data: recibos } = await client.from("vendas_recibos").select("id, venda_id, numero_recibo, numero_recibo_normalizado, vendas!inner(company_id)").or(`numero_recibo.eq.${documento},numero_recibo_normalizado.eq.${docUpper}`).eq("vendas.company_id", companyId).limit(1);
    const recibo = recibos?.[0];
    if (!recibo) {
      semMatch++;
      continue;
    }
    const { error: updateError } = await client.from("conciliacao_recibos").update({
