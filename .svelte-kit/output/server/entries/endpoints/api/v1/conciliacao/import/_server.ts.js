...
async function POST(event) {
  try {
    ...
    const importaveis = linhas.filter(
      (linha) => isConciliacaoImportavel({ status: linha.status, descricao: linha.descricao })
    );
    ...
    const { data: existentes } = await client.from("conciliacao_recibos").select("id, documento, movimento_data, descricao").eq("company_id", companyId).in("documento", importaveis.map((l) => l.documento)).limit(5e3);
    const existentesKeys = new Set(
      (existentes || []).map((row) => buildImportKey(companyId, row.movimento_data, row.documento, row.descricao))
    );
