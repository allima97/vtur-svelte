...
async function POST(event) {
  try {
    ...
    const documentos = Array.isArray(body?.documentos) ? body.documentos.map((d) => String(d || "").trim()).filter(Boolean) : [];
    if (documentos.length === 0) return json({ records: {} });
    const { data, error } = await client.from("conciliacao_recibos").select(
    ...
    ).eq("company_id", companyId).in("documento", documentos).order("movimento_data", { ascending: false }).limit(1e3);
    ...
    for (const row of data || []) {
      ...
      const rowHasFinancial = Math.abs(Number(row?.valor_lancamentos || 0)) > 1e-3 || Math.abs(Number(row?.valor_taxas || 0)) > 1e-3;
      if (existing) {
        if (existing.conciliado && !row?.conciliado) continue;
        const existingHasFinancial = Math.abs(Number(existing.valor_lancamentos || 0)) > 1e-3 || Math.abs(Number(existing.valor_taxas || 0)) > 1e-3;
