...
import { g as getAdminClient, r as requireAuthenticatedUser, a as resolveUserScope, e as ensureModuloAccess, b as resolveScopedCompanyIds, n as normalizeText, t as toErrorResponse, i as isUuid } from "../../../../../chunks/v1.js";
async function GET(event) {
  try {
    ...
    let query = client.from("vendas_pagamentos").select(`
      ...
      `).order("created_at", { ascending: false }).limit(2e3);
...
async function POST(event) {
  try {
    ...
    if (!scope.isAdmin) {
      const { data: venda, error: vendaErr } = await client.from("vendas").select("id, company_id").eq("id", body.venda_id).maybeSingle();
    ...
    const { data, error } = await client.from("vendas_pagamentos").insert([{
