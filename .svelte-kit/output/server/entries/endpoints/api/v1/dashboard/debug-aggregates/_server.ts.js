...
async function GET(event) {
  try {
    ...
    ).eq("company_id", companyId).eq("cancelada", false).gte("data_venda", inicio).lte("data_venda", fim);
    if (vendedorId) {
    ...
    const { data: concRecords, error: concErr } = await client.from("conciliacao_recibos").select(
    ...
    ).eq("company_id", companyId).neq("is_baixa_rac", true).gte("movimento_data", inicio).lte("movimento_data", fim).order("movimento_data", { ascending: true });
    if (concErr) throw concErr;
    let concBaixa = 0;
    let concOpfax = 0;
    let concOther = 0;
    let concLinked = 0;
    const relevantDocs = /* @__PURE__ */ new Set();
    (concRecords || []).forEach((row) => {
    ...
    return json({
      ...
      conciliacao: {
        ...
        count_baixa: concRecords?.filter((r) => String(r.status || "").toUpperCase() === "BAIXA").length,
        count_opfax: concRecords?.filter((r) => String(r.status || "").toUpperCase() === "OPFAX").length,
        count_other_status: (concRecords || []).length - (concRecords?.filter(
          (r) => String(r.status || "").toUpperCase() === "BAIXA" || String(r.status || "").toUpperCase() === "OPFAX"
        ).length || 0),
        count_null_status: concRecords?.filter((r) => !r.status || String(r.status || "").trim() === "").length,
