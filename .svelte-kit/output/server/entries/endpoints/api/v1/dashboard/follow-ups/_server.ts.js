...
import { g as getDefaultFollowUpRange, i as isIsoDate, r as resolveFollowUpFilters } from "../../../../../../chunks/agenda.js";
...
async function GET(event) {
  try {
    ...
    let candidatasQuery = client.from("viagens").select(
      `
          id,
          venda_id,
          company_id,
        ...
        `
    ).not("data_fim", "is", null).gte("data_fim", inicio).lte("data_fim", fim).or("status.is.null,status.neq.Fechado").eq("venda.cancelada", false).order("data_fim", { ascending: false }).limit(500);
    ...
    const vendaIds = Array.from(
      new Set(
        (candidatasData || []).map((row) => String(row?.venda_id || row?.venda?.id || "").trim()).filter(Boolean)
      )
    );
    const avulsas = (candidatasData || []).filter((row) => !row?.venda_id);
    let detalhadas = [];
    if (vendaIds.length > 0) {
      let detalhadasQuery = client.from("viagens").select(
        `
            id,
            venda_id,
            company_id,
          ...
          `
      ).in("venda_id", vendaIds).not("data_fim", "is", null).or("status.is.null,status.neq.Fechado").eq("venda.cancelada", false).order("data_fim", { ascending: false }).limit(5e3);
      if (companyIds.length > 0) {
    ...
    const items = Array.from(grupos.values()).filter((item) => {
    ...
    }).sort((a, b) => String(b?.data_fim || "").localeCompare(String(a?.data_fim || ""))).map((item) => ({
      id: String(item.id),
