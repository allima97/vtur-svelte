...
async function loadDossie(client, viagemId, companyId, userId, usoIndividual) {
  let query = client.from("viagens").select(`
    ...
    `).eq("id", viagemId).eq("company_id", companyId);
  if (usoIndividual) {
  ...
  let viagensVenda = [];
  if (detalhe?.venda_id) {
    let viagensQuery = client.from("viagens").select("id, recibo_id, origem, destino, status, data_inicio, data_fim, observacoes").eq("venda_id", detalhe.venda_id).eq("company_id", companyId);
    if (usoIndividual) {
  ...
  let acompanhantesCliente = [];
  const clienteBaseId = detalhe?.venda?.cliente_id || null;
  if (clienteBaseId) {
    const { data } = await client.from("cliente_acompanhantes").select("id, nome_completo, cpf, telefone, grau_parentesco, data_nascimento").eq("cliente_id", clienteBaseId).eq("ativo", true).order("nome_completo", { ascending: true });
...
async function POST(event) {
  try {
    ...
    let baseQuery = client.from("viagens").select("id, company_id, venda_id, cliente_id, responsavel_user_id").eq("id", viagemId).eq("company_id", scope.companyId || "");
    ...
    if (action === "acompanhante_save") {
      ...
      let updateQuery = client.from("viagens").update({
