...
async function GET(event) {
  try {
    ...
    const { data: viagem, error } = await client.from("viagens").select(`
        id,
        venda_id,
        orcamento_id,
        cliente_id,
    ...
    if (viagem.cliente_id) {
      const { data: clienteData } = await client.from("clientes").select("id, nome, email, telefone, whatsapp").eq("id", viagem.cliente_id).single();
      cliente = clienteData;
    }
    let venda = null;
    if (viagem.venda_id) {
      const { data: vendaData } = await client.from("vendas").select("id, valor_total, valor_total_pago, status, data_venda").eq("id", viagem.venda_id).single();
      if (vendaData) {
        venda = { ...vendaData };
        const { data: recibosData } = await client.from("vendas_recibos").select(`
            id,
            produto_id,
        ...
        const produtoIds = [...new Set((recibosData || []).map((r) => r.produto_id || r.produto_resolvido_id).filter(Boolean))];
        const produtosMap = /* @__PURE__ */ new Map();
        if (produtoIds.length > 0) {
          const { data: produtosData } = await client.from("produtos").select("id, nome").in("id", produtoIds);
    ...
    let recibo = null;
    if (viagem.recibo_id) {
      const { data: reciboData } = await client.from("vendas_recibos").select("id, numero_recibo, numero_reserva, valor_total, data_inicio, data_fim").eq("id", viagem.recibo_id).single();
      recibo = reciboData;
    }
    const { data: vouchers } = viagem.venda_id ? await client.from("vouchers").select("id, nome, provider, codigo_systur, codigo_fornecedor, data_inicio, data_fim, ativo").eq("company_id", viagem.company_id).limit(20) : { data: [] };
    const { data: passageiros } = await client.from("viagem_passageiros").select(`
      ...
      `).eq("viagem_id", id).order("created_at", { ascending: true });
    return json({
...
async function PATCH(event) {
  try {
    ...
    const { data: existing, error: checkError } = await client.from("viagens").select("id, company_id").eq("id", id).single();
    ...
    if (scope.usoIndividual) {
      const { data: ownCheck } = await client.from("viagens").select("id").eq("id", id).eq("responsavel_user_id", user.id).maybeSingle();
      if (!ownCheck?.id) {
        return json({ error: "Sem acesso a esta viagem" }, { status: 403 });
      }
    }
    ...
    allowedFields.forEach((field) => {
      if (body[field] !== void 0) updateData[field] = body[field];
    });
    const { data, error } = await client.from("viagens").update(updateData).eq("id", id).select().single();
