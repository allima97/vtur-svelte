...
import { g as getAdminClient, r as requireAuthenticatedUser, a as resolveUserScope, e as ensureModuloAccess, i as isUuid, h as fetchGestorEquipeIdsComGestor, t as toErrorResponse } from "../../../../../../../chunks/v1.js";
...
async function POST(event) {
  try {
    ...
    const rawId = ajusteId ? ajusteId.replace(/^vr:/, "").replace(/^cr:/, "").trim() : vendaReciboIdRaw;
    if (!isUuid(rawId)) {
    ...
    let keyColumn = "venda_recibo_id";
    if (!isConciliacao) {
      const { data: reciboRow, error: reciboError } = await client.from("vendas_recibos").select(
        `
            id,
            venda_id,
            vendas!inner (
          ...
          `
      ).eq("id", rawId).eq("vendas.cancelada", false).maybeSingle();
      if (reciboError) throw reciboError;
      if (!reciboRow) return json({ error: "Recibo nao encontrado." }, { status: 404 });
      const reciboCompany = String(reciboRow?.vendas?.company_id || "").trim();
      if (!scope.isAdmin && companyId && reciboCompany !== companyId) {
        return json({ error: "Recibo fora do escopo da empresa." }, { status: 403 });
      }
      vendedorOrigemId = String(reciboRow?.vendas?.vendedor_id || "").trim();
      keyColumn = "venda_recibo_id";
    } else {
      const { data: concRow, error: concErr } = await client.from("conciliacao_recibos").select("id, company_id, ranking_vendedor_id, venda_id").eq("id", rawId).maybeSingle();
      if (concErr) throw concErr;
      if (!concRow) return json({ error: "Recibo da conciliacao nao encontrado." }, { status: 404 });
      const concCompany = String(concRow?.company_id || "").trim();
      ...
      vendedorOrigemId = String(concRow?.ranking_vendedor_id || "").trim();
      if (!isUuid(vendedorOrigemId)) {
        const vendaId = String(concRow?.venda_id || "").trim();
        if (isUuid(vendaId)) {
          const { data: vendaRow, error: vendaErr } = await client.from("vendas").select("id, vendedor_id, company_id, cancelada").eq("id", vendaId).eq("cancelada", false).maybeSingle();
          if (vendaErr) throw vendaErr;
          vendedorOrigemId = String(vendaRow?.vendedor_id || "").trim();
        }
      }
    ...
    if (percentualDestino === 0) {
      const { error: deleteError } = await client.from("vendas_recibos_rateio").delete().eq(keyColumn, rawId).eq("company_id", companyId ?? "");
    ...
    const { data: destinoUser, error: destinoError } = await client.from("users").select("id, company_id, active").eq("id", vendedorDestinoId).eq("active", true).maybeSingle();
    ...
    const { data: updatedRows, error: updateError } = await client.from("vendas_recibos_rateio").update(payload).eq(keyColumn, rawId).eq("company_id", companyId ?? "").select("id");
