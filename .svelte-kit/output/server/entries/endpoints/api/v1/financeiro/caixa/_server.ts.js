...
import { g as getAdminClient, r as requireAuthenticatedUser, a as resolveUserScope, e as ensureModuloAccess, b as resolveScopedCompanyIds, c as toISODateLocal, j as getMonthRange, t as toErrorResponse } from "../../../../../../chunks/v1.js";
async function GET(event) {
  try {
    ...
    if (companyIds.length > 0) {
      const { data: vendasScope } = await client.from("vendas").select("id").in("company_id", companyIds).gte("data_venda", inicio).lte("data_venda", fim).limit(5e3);
      vendaIds = (vendasScope || []).map((v) => String(v.id || "").trim()).filter(Boolean);
    ...
    let pagamentosQuery = client.from("vendas_pagamentos").select("id, venda_id, forma_nome, valor_total, created_at, venda:vendas!venda_id(numero_venda, cliente_id, data_venda, company_id)").gte("created_at", inicio + "T00:00:00").lte("created_at", fim + "T23:59:59");
    if (vendaIds.length > 0) {
    ...
    let movQuery = client.from("caixa_movimentacoes").select("*, forma_pagamento:forma_pagamento_id(id, nome)").gte("data_movimentacao", inicio).lte("data_movimentacao", fim).order("data_movimentacao", { ascending: false });
    ...
    const totalEntradasMovimentacoes = movItems.filter((m) => String(m.tipo || "").toLowerCase() === "entrada").reduce((sum, m) => sum + Number(m.valor || 0), 0);
    const totalSaidas = movItems.filter((m) => String(m.tipo || "").toLowerCase() === "saida").reduce((sum, m) => sum + Number(m.valor || 0), 0);
    const totalEntradas = totalEntradasPagamentos + totalEntradasMovimentacoes;
    const saldo = totalEntradas - totalSaidas;
    const porFormaPagamento = /* @__PURE__ */ new Map();
    pagItems.forEach((p) => {
...
async function POST(event) {
  try {
    ...
    if (!body.tipo || !body.descricao || body.valor === void 0 || !body.data_movimentacao) {
      return json({ success: false, error: "Tipo, descricao, valor e data sao obrigatorios." }, { status: 400 });
    }
    const { data, error } = await client.from("caixa_movimentacoes").insert([{
