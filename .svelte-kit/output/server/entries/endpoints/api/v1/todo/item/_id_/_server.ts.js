...
async function GET(event) {
  try {
    ...
    const { data, error } = await client.from("agenda_itens").select("id, titulo, descricao, done, categoria_id, prioridade, status, arquivo, created_at, updated_at, user_id, tipo").eq("id", id).maybeSingle();
    ...
    if (data.categoria_id) {
      const { data: categoriaData } = await client.from("todo_categorias").select("id, nome, cor").eq("id", data.categoria_id).maybeSingle();
      if (categoriaData) {
        categoria = {
