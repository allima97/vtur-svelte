...
import { c as getCurrentYearRange, f as fetchSalesReportRows, k as fetchLatestPaymentForms, h as getVendaDestino, j as getReceiptProductDescriptor, i as getVendaStatus, g as getVendaCommission, a as getVendaVendedorNome, b as getVendaClienteNome, l as getVendaCodigo } from "../../../../../../chunks/relatorios.js";
async function GET(event) {
  try {
    ...
    let items = filteredRows.map((row) => {
      ...
      const recibos = (row.recibos || []).map((recibo) => {
        const descriptor = getReceiptProductDescriptor(recibo, row);
        return {
          id: recibo?.id || null,
    ...
    const vendedores = Array.from(
      new Map(
        filteredRows.filter((row) => row.vendedor_id).map((row) => [row.vendedor_id, getVendaVendedorNome(row)])
      ).entries()
    ).map(([id, nome]) => ({ id, nome })).sort((left, right) => left.nome.localeCompare(right.nome, "pt-BR"));
