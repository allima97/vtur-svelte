...
import { g as getAdminClient, r as requireAuthenticatedUser, a as resolveUserScope, e as ensureModuloAccess, b as resolveScopedCompanyIds, d as resolveScopedVendedorIds, j as getMonthRange, t as toErrorResponse } from "../../../../../../chunks/v1.js";
import { c as getCurrentYearRange, f as fetchSalesReportRows, a as getVendaVendedorNome, g as getVendaCommission } from "../../../../../../chunks/relatorios.js";
function getPreviousPeriod(dataInicio, dataFim) {
  const start = /* @__PURE__ */ new Date(`${dataInicio}T00:00:00`);
  const end = /* @__PURE__ */ new Date(`${dataFim}T00:00:00`);
  const diffDays = Math.max(1, Math.round((end.getTime() - start.getTime()) / 864e5) + 1);
...
function normalizeTendencia(currentValue, previousValue) {
...
async function GET(event) {
  try {
    ...
    if (vendedorIds.length === 0 && companyIds.length > 0) {
      const { data: companyUsers, error: companyUsersError } = await client.from("users").select("id").in("company_id", companyIds).limit(1e3);
      if (companyUsersError) throw companyUsersError;
      vendedorIds = (companyUsers || []).map((row) => String(row?.id || "").trim()).filter(Boolean);
    }
    const [rows, previousRows, quotesRes, metasRes] = await Promise.all([
      ...
      (async () => {
        let query = client.from("quote").select("id, created_by, total").gte("created_at", `${dataInicio}T00:00:00`).lte("created_at", `${dataFim}T23:59:59.999`).limit(5e3);
      ...
      (async () => {
        let query = client.from("metas_vendedor").select("id, vendedor_id, meta_geral, meta_diferenciada, periodo, ativo").eq("ativo", true).gte("periodo", dataInicio).lte("periodo", dataFim).limit(1e3);
        if (vendedorIds.length > 0) {
    ...
    const previousRevenueMap = /* @__PURE__ */ new Map();
    rows.forEach((row) => {
    ...
    const missingNameIds = Array.from(rankingMap.values()).filter((item) => item.vendedor_nome === "Equipe VTUR").map((item) => item.vendedor_id);
    if (missingNameIds.length > 0) {
      const { data: usersData, error: usersError } = await client.from("users").select("id, nome_completo, email").in("id", missingNameIds);
    ...
    let items = Array.from(rankingMap.values()).map((item) => {
      const ticketMedio = item.total_vendas > 0 ? item.total_receita / item.total_vendas : 0;
      const taxaConversao = item.total_orcamentos > 0 ? item.total_vendas / item.total_orcamentos * 100 : 0;
    ...
    }).sort((left, right) => right.total_receita - left.total_receita).map((item, index) => ({
      ...item,
      posicao: index + 1,
