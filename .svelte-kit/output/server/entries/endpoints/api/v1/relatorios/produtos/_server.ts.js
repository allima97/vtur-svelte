...
import { c as getCurrentYearRange, f as fetchSalesReportRows, j as getReceiptProductDescriptor, g as getVendaCommission } from "../../../../../../chunks/relatorios.js";
async function GET(event) {
  try {
    ...
    const byProduto = /* @__PURE__ */ new Map();
    rows.forEach((row) => {
      ...
      recibos.forEach((recibo) => {
        ...
        const lucroRecibo = Number(recibo?.valor_taxas || 0) + Number(recibo?.valor_du || 0) + Number(recibo?.valor_rav || 0);
        ...
        current.lucro += lucro;
        current.produto_id = row.produto_id ?? null;
        byProduto.set(key, current);
      });
    });
    const items = Array.from(byProduto.values()).map((item) => ({
      ...item,
      produto_id: typeof item.produto_id !== "undefined" ? item.produto_id : null,
