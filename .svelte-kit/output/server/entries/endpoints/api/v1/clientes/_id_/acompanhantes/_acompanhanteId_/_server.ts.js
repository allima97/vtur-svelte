...
async function PATCH(event) {
  try {
    ...
    const { data, error } = await client.from("cliente_acompanhantes").update({
    ...
    }).eq("id", acompanhanteId).eq("cliente_id", clienteId).select("id, cliente_id, company_id, nome_completo, cpf, telefone, grau_parentesco, rg, data_nascimento, observacoes, ativo, created_at, updated_at").single();
...
async function DELETE(event) {
  try {
    ...
    const { error } = await client.from("cliente_acompanhantes").delete().eq("id", acompanhanteId).eq("cliente_id", clienteId);
    if (error) throw error;
    return json({
