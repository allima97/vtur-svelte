...
import { b as loadEmailSettings, c as buildFromEmails } from "../../../../../../../chunks/admin.js";
...
function applyVars(text, vars) {
  return String(text || "").replace(/\{nome\}/gi, vars.nome || "").replace(/\{nome_completo\}/gi, vars.nome_completo || "").replace(/\{email\}/gi, vars.email || "");
}
function renderHtml(text) {
  return String(text || "").split(/\r?\n/).map((line) => `<p>${line || "&nbsp;"}</p>`).join("");
}
async function tryInsertHistorico(client, payload) {
...
async function POST(event) {
  try {
    ...
    const { data: cliente, error: clienteError } = await client.from("clientes").select("id, nome, email, telefone, whatsapp, nascimento").eq("id", clienteId).maybeSingle();
    ...
    if (canal === "email") {
      ...
      const historicoDisponivel2 = await tryInsertHistorico(client, {
      ...
      return json({
        ...
        cliente_id: clienteId,
        template_id: templateId || null,
        historico_disponivel: historicoDisponivel2
      });
    }
