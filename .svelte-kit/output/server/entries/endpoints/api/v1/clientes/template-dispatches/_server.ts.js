...
const GET = async ({ locals, url }) => {
  try {
    ...
    let query = client.from("cliente_template_dispatches").select("*").order("sent_at", { ascending: false }).limit(limit);
...
const POST = async ({ locals, request }) => {
  try {
    ...
    const { data, error } = await client.from("cliente_template_dispatches").insert(payload).select().single();
    if (error) throw error;
