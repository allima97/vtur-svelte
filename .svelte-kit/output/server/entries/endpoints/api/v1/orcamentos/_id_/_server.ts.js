...
function applyQuoteScope(query, scope, user, vendedorIds) {
...
async function GET(event) {
  try {
    ...
    const { data: items } = await client.from("quote_item").select("*").eq("quote_id", id).order("order_index", { ascending: true });
    let cliente = null;
    if (quote.client_id) {
      const { data: clienteData } = await client.from("clientes").select("id, nome, email, telefone").eq("id", quote.client_id).maybeSingle();
      cliente = clienteData;
    }
    let vendedor = "Equipe VTUR";
    if (quote.created_by) {
      const { data: vendedorData } = await client.from("users").select("nome_completo").eq("id", quote.created_by).maybeSingle();
...
async function PATCH(event) {
  try {
    ...
    if (body.total !== void 0) updateData.total = body.total;
    if (body.currency !== void 0) updateData.currency = body.currency;
    if (body.client_id !== void 0) updateData.client_id = body.client_id;
    if (body.data_embarque !== void 0) updateData.data_embarque = body.data_embarque;
    if (body.data_final !== void 0) updateData.data_final = body.data_final;
