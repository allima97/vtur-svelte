...
function validateItem(item) {
  return Boolean(
    item.item_type && item.quantity > 0 && item.start_date && item.title && Number(item.total_amount || 0) > 0
  );
}
async function buildTipoLabelMap(client) {
  const { data, error: err } = await client.from("tipo_produtos").select("id, nome, tipo").order("nome", { ascending: true }).limit(500);
  if (err) throw err;
...
async function syncProductsCatalog(client, items) {
  ...
  for (const item of items) {
    ...
    try {
      ...
      if (existing?.id) {
        const { error: updateErr } = await client.from("produtos").update(payload).eq("id", existing.id);
...
async function POST(event) {
  try {
    ...
    const removedItemIds = Array.isArray(body?.removed_item_ids) ? body.removed_item_ids : [];
    ...
    const { error: itemError } = await client.from("quote_item").upsert(payload, { onConflict: "id" });
    if (itemError) throw itemError;
    if (removedItemIds.length) {
      const { error: deleteRemovedSegs } = await client.from("quote_item_segment").delete().in("quote_item_id", removedItemIds);
      if (deleteRemovedSegs) throw deleteRemovedSegs;
      const { error: deleteRemovedItems } = await client.from("quote_item").delete().in("id", removedItemIds);
      if (deleteRemovedItems) throw deleteRemovedItems;
    }
    const itemIds = payload.map((item) => item.id).filter(Boolean);
    if (itemIds.length) {
      const { error: deleteSegErr } = await client.from("quote_item_segment").delete().in("quote_item_id", itemIds);
      if (deleteSegErr) throw deleteSegErr;
      const segmentPayloads = items.flatMap(
        (item) => (item.segments || []).map((segment, idx) => ({
      ...
      if (segmentPayloads.length) {
        const { error: segErr } = await client.from("quote_item_segment").insert(segmentPayloads);
    ...
    const { error: quoteError } = await client.from("quote").update(quotePayload).eq("id", quoteId);
