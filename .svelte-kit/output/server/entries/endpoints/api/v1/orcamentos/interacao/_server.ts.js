...
async function POST(event) {
  try {
    ...
    let updateQuery = client.from("quote").update(updateData).eq("id", quoteId);
    ...
    const { data, error } = await updateQuery.select("id, status_negociacao, last_interaction_at, last_interaction_notes").single();
...
async function GET(event) {
  try {
    ...
    const companyIds = resolveScopedCompanyIds(scope, null);
    const vendedorIds = await resolveScopedVendedorIds(client, scope, null);
    let query = client.from("quote").select("id, status_negociacao, last_interaction_at, last_interaction_notes, updated_at").eq("id", quoteId);
