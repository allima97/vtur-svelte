...
import { g as getAdminClient, r as requireAuthenticatedUser, a as resolveUserScope, e as ensureModuloAccess, n as normalizeText, i as isUuid, b as resolveScopedCompanyIds, d as resolveScopedVendedorIds, t as toErrorResponse } from "../../../../../../chunks/v1.js";
function getResumo(recibo, venda) {
  ...
  const detalhes = [
    ...
    typeof recibo?.valor_total === "number" ? new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(recibo.valor_total) : ""
  ].filter(Boolean).join(" - ");
...
async function GET(event) {
  try {
    ...
    let saleQuery = client.from("vendas").select("id, cliente_id, vendedor_id, company_id").eq("id", vendaId);
    ...
    const { data: currentReceiptsData, error: currentReceiptsError } = await client.from("vendas_recibos").select("id").eq("venda_id", vendaId);
    if (currentReceiptsError) throw currentReceiptsError;
    const currentReceiptIds = (currentReceiptsData || []).map((row) => String(row?.id || "").trim()).filter(Boolean);
    const { data: currentLinksData, error: currentLinksError } = await client.from("vendas_recibos_complementares").select("id, venda_id, recibo_id").eq("venda_id", vendaId);
    ...
    const linkedReceiptIds = currentLinks.map((row) => String(row?.recibo_id || "").trim()).filter(Boolean);
    const { data: linkedReceiptsData, error: linkedReceiptsError } = linkedReceiptIds.length > 0 ? await client.from("vendas_recibos").select(`
              id,
              venda_id,
              numero_recibo,
              valor_total,
              produto_resolvido:produtos!produto_resolvido_id (nome)
            `).in("id", linkedReceiptIds) : { data: [], error: null };
    if (linkedReceiptsError) throw linkedReceiptsError;
    const linkedSalesIds = Array.from(
      new Set(
        (linkedReceiptsData || []).map((row) => String(row?.venda_id || "").trim()).filter(Boolean)
      )
    );
    const { data: linkedSalesData, error: linkedSalesError } = linkedSalesIds.length > 0 ? await client.from("vendas").select(`
              id,
              cliente_id,
              destino_id,
              destino_cidade_id,
            ...
            `).in("id", linkedSalesIds) : { data: [], error: null };
    ...
    const { data: pairReceiptsData, error: pairReceiptsError } = pairSaleIds.length > 0 ? await client.from("vendas_recibos").select("id, venda_id").in("venda_id", [vendaId, ...pairSaleIds]) : { data: [], error: null };
    if (pairReceiptsError) throw pairReceiptsError;
    const receiptsBySale = /* @__PURE__ */ new Map();
    for (const row of pairReceiptsData || []) {
      ...
      const current2 = receiptsBySale.get(saleRef) || [];
      current2.push(receiptRef);
      receiptsBySale.set(saleRef, current2);
    }
    const { data: allPairLinksData, error: allPairLinksError } = pairSaleIds.length > 0 ? await client.from("vendas_recibos_complementares").select("id, venda_id, recibo_id").in("venda_id", [vendaId, ...pairSaleIds]) : { data: [], error: null };
    ...
    const current = currentLinks.map((link) => {
      ...
      const relatedIds = allPairLinks.filter((row) => {
    ...
    let suggestions = [];
    if (busca.length >= 2) {
      let receiptsQuery = client.from("vendas_recibos").select(`
          id,
          venda_id,
          numero_recibo,
          valor_total,
      ...
      suggestions = (scopedReceiptsData || []).map((row) => {
      ...
      }).filter((item) => item.venda_id && item.venda_id !== vendaId).filter((item) => !currentLinkedIds.has(item.recibo_id)).filter((item) => {
