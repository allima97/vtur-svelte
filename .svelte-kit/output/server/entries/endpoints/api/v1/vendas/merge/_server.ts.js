...
import { g as getAdminClient, r as requireAuthenticatedUser, a as resolveUserScope, e as ensureModuloAccess, i as isUuid, b as resolveScopedCompanyIds, d as resolveScopedVendedorIds, h as fetchGestorEquipeIdsComGestor, t as toErrorResponse } from "../../../../../../chunks/v1.js";
...
function buildPagamentoKey(pagamento) {
  const forma = String(pagamento?.forma_nome || pagamento?.forma_pagamento_id || "").toLowerCase().trim();
  ...
  const parcelasKey = parcelas.map((parcela) => {
  ...
  const pagaComissaoKey = pagamento?.paga_comissao === null || pagamento?.paga_comissao === void 0 ? "na" : pagamento.paga_comissao ? "1" : "0";
  return [forma, totalKey, parcelasKey, String(parcelasQtd), pagaComissaoKey].join("|");
}
function dedupePagamentos(pagamentos, preferVendaId) {
...
function calcularValorPagamento(pagamento) {
...
async function carregarTermosNaoComissionaveis(client) {
  try {
    const { data, error } = await client.from("parametros_pagamentos_nao_comissionaveis").select("termo, termo_normalizado, ativo").eq("ativo", true).order("termo", { ascending: true });
    if (error) throw error;
    const termos = (data || []).map((row) => normalizeText(row?.termo_normalizado || row?.termo)).filter(Boolean);
...
function parseBodyIds(value) {
  ...
  return String(value || "").split(",").map((item) => item.trim()).filter((item) => isUuid(item));
}
async function POST(event) {
  try {
    ...
    let salesQuery = client.from("vendas").select("id, vendedor_id, desconto_comercial_aplicado, desconto_comercial_valor, data_embarque, data_final, company_id").in("id", saleIds);
    ...
    const foundIds = new Set(sales.map((item) => String(item?.id || "")));
    if (saleIds.some((id) => !foundIds.has(id))) {
      return new Response("Vendas invalidas para mescla.", { status: 404 });
    }
    const { data: receiptsData, error: receiptsError } = await client.from("vendas_recibos").select("id, venda_id, valor_total, valor_taxas, data_inicio, data_fim").in("venda_id", saleIds);
    if (receiptsError) throw receiptsError;
    const { data: paymentsData, error: paymentsError } = await client.from("vendas_pagamentos").select("id, venda_id, forma_pagamento_id, forma_nome, valor_total, valor_bruto, desconto_valor, parcelas, parcelas_qtd, parcelas_valor, paga_comissao, operacao, plano").in("venda_id", saleIds);
    ...
    if (duplicateIds.length > 0) {
      const { error: deleteDuplicatePaymentsError } = await client.from("vendas_pagamentos").delete().in("id", duplicateIds);
      if (deleteDuplicatePaymentsError) throw deleteDuplicatePaymentsError;
    }
    if (mergeIds.length > 0) {
      const { error: updatePaymentsError } = await client.from("vendas_pagamentos").update({ venda_id: vendaId }).in("venda_id", mergeIds);
      if (updatePaymentsError) throw updatePaymentsError;
      const { error: updateReceiptNotesError } = await client.from("vendas_recibos_notas").update({ venda_id: vendaId }).in("venda_id", mergeIds);
      if (updateReceiptNotesError && String(updateReceiptNotesError.code || "") !== "42P01") {
        throw updateReceiptNotesError;
      }
      const { error: updateReceiptsError } = await client.from("vendas_recibos").update({ venda_id: vendaId }).in("venda_id", mergeIds);
      if (updateReceiptsError) throw updateReceiptsError;
      const { error: updateTripsError } = await client.from("viagens").update({ venda_id: vendaId }).in("venda_id", mergeIds);
    ...
    const totalBrutoRecibos = receipts.reduce((acc, item) => acc + Number(item?.valor_total || 0), 0);
    const totalTaxasRecibos = receipts.reduce((acc, item) => acc + Number(item?.valor_taxas || 0), 0);
    ...
    const valorNaoComissionado = deduped.reduce((acc, pagamento) => {
    ...
    const valorComissionavel = totalPagoFinal > 0 ? Math.max(0, totalPagoFinal - valorNaoComissionado) : 0;
    const descontoTotal = sales.reduce(
      (acc, item) => acc + Number(item?.desconto_comercial_valor || 0),
      0
    );
    const principalSale = sales.find((item) => item?.id === vendaId);
    const dataEmbarque = receipts.map((item) => String(item?.data_inicio || "").trim()).filter(Boolean).sort()[0] || principalSale?.data_embarque || null;
    const datasFim = receipts.map((item) => String(item?.data_fim || "").trim()).filter(Boolean).sort();
    const dataFinal = datasFim.length > 0 ? datasFim[datasFim.length - 1] : principalSale?.data_final || null;
    let updateSaleQuery = client.from("vendas").update({
