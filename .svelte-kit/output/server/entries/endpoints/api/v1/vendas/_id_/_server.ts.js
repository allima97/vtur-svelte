...
import { g as getAdminClient, r as requireAuthenticatedUser, a as resolveUserScope, e as ensureModuloAccess, b as resolveScopedCompanyIds, d as resolveScopedVendedorIds, t as toErrorResponse, i as isUuid } from "../../../../../../chunks/v1.js";
import { e as ensureAssignableActiveSeller, a as ensureReciboReservaUnicos, b as buildVendaPayload, s as syncVendaChildren } from "../../../../../../chunks/vendasSave.js";
async function GET(event) {
  try {
    ...
    let query = client.from("vendas").select(`*, cliente:clientes!vendas_cliente_id_fkey(id,nome), vendedor:users!vendas_vendedor_id_fkey(id,nome_completo), destino:produtos!vendas_destino_id_fkey(id,nome), recibos:vendas_recibos(*), pagamentos:vendas_pagamentos(*)`).eq("id", id);
...
async function PATCH(event) {
  try {
    ...
    const user = await requireAuthenticatedUser(event);
    const scope = await resolveUserScope(client, user.id);
    if (!scope.isAdmin) ensureModuloAccess(scope, ["vendas"], 3, "Sem permissão para editar vendas.");
    const id = String(event.params.id || "").trim();
