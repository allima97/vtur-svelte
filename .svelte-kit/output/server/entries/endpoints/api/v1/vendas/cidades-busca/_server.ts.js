...
async function GET(event) {
  try {
    ...
    if (cidadeId) {
      let data = null;
      const detailed = await client.from("cidades").select("id, nome, subdivisao:subdivisoes(nome, codigo_admin1)").eq("id", cidadeId).maybeSingle();
      if (detailed.error) {
    ...
    try {
      ...
      const fallbackWithSubdivisao = await client.from("cidades").select("id, nome, subdivisao:subdivisoes(nome, codigo_admin1)").ilike("nome", `%${query}%`).order("nome", { ascending: true }).limit(limite);
      if (fallbackWithSubdivisao.error) {
        const fallbackBase = await client.from("cidades").select("id, nome").ilike("nome", `%${query}%`).order("nome", { ascending: true }).limit(limite);
    ...
    const filtered = Array.from(dedup.values()).filter((item) => normalizeText(`${item.nome} ${item.subdivisao_nome || ""} ${item.pais_nome || ""}`).includes(normalizedQuery)).slice(0, limite);
