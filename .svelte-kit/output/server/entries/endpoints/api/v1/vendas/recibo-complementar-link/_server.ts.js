...
async function POST(event) {
  try {
    ...
    if (Array.isArray(body?.links)) {
      ...
      const links = body.links.filter((item) => isUuid(String(item?.venda_id || "").trim()) && isUuid(String(item?.recibo_id || "").trim())).map((item) => ({
      ...
      const { error: batchError } = await client.from("vendas_recibos_complementares").upsert(links, { onConflict: "venda_id,recibo_id", ignoreDuplicates: true });
    ...
    const { error: primaryLinkError } = await client.from("vendas_recibos_complementares").upsert(primaryLink, { onConflict: "venda_id,recibo_id", ignoreDuplicates: true });
    ...
    if (!cruzadoJaVinculado && vendaCruzadaId && reciboCruzadoId) {
      ...
      const { error: crossLinkError } = await client.from("vendas_recibos_complementares").upsert(crossLink, { onConflict: "venda_id,recibo_id", ignoreDuplicates: true });
      if (crossLinkError) {
