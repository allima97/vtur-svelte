...
async function GET(event) {
  try {
    ...
    let currentSaleQuery = client.from("vendas").select("id, cliente_id, vendedor_id, company_id").eq("id", vendaId);
    ...
    let query = client.from("vendas").select(`
      ...
      `).eq("cliente_id", currentSale.cliente_id).eq("vendedor_id", currentSale.vendedor_id).neq("id", currentSale.id).order("data_venda", { ascending: false });
    ...
    const saleIds = (salesData || []).map((row) => String(row?.id || "").trim()).filter(Boolean);
    const receiptsBySale = /* @__PURE__ */ new Map();
    if (saleIds.length > 0) {
      const { data: receiptsData, error: receiptsError } = await client.from("vendas_recibos").select("venda_id, numero_recibo").in("venda_id", saleIds).order("numero_recibo", { ascending: true });
